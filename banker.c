...
bool resource_request(int process_id) {
    ...
    // Check if this request fulfills the mission entirely
    ...
    if (is_safe(safe_seq) || process_completes) {
        ...
        if (process_completes) {
            printf("[MISSION ACCOMPLISHED] %s is returning all assets to base!\n", mission_names[process_id]);
            for (int i = 0; i < num_resources; i++) {
...
void init_random_system() {
    ...
    for (int i = 0; i < num_resources; i++) available[i] = 5 + rand() % 10; 
    for (int i = 0; i < num_processes; i++) {
        for (int j = 0; j < num_resources; j++) {
            max[i][j] = 3 + rand() % 5;
